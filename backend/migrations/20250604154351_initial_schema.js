...
COMMENT ON COLUMN auth_tokens.token_hash IS 'Hashed value of the token for security';
  `);
  // Example of adding a comment to the migration, can be seen with pgm.verbose() if logger is configured
  pgm.addComment('Initial schema setup including users, user_profiles, and auth_tokens tables.');
};

exports.down = pgm => {
  // Order matters for dropping: dependent tables first, then the tables they reference.
  // Also drop functions and triggers if they were created.
  pgm.sql(\`
  ...
  \`);
  pgm.log('Rolled back initial schema: dropped users, user_profiles, auth_tokens tables and trigger_set_timestamp function.');
};
