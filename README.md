...
## راه‌اندازی کل سایت با یک دستور (Docker)

برای راه‌اندازی سریع و آسان کل پلتفرم (شامل بک‌اند، فرانت‌اند و دیتابیس) با استفاده از Docker، می‌توانید از اسکریپت `start-site.sh` که در ریشه پروژه قرار دارد، استفاده کنید. این اسکریپت به طور خودکار موارد زیر را انجام می‌دهد:

*   بیلد کردن ایمیج‌های Docker برای سرویس‌های بک‌اند و فرانت‌اند (در صورت نیاز).
*   اجرای migrationهای دیتابیس برای آماده‌سازی ساختار جداول.
*   اجرای تمامی سرویس‌ها در پس‌زمینه.

**دستور اجرا:**

ابتدا مطمئن شوید که در ریشه پروژه قرار دارید و سپس دستور زیر را اجرا کنید:
...
*   فرانت‌اند سایت از طریق آدرس `http://localhost:3000` در دسترس خواهد بود.
*   بک‌اند API (در صورت نیاز به دسترسی مستقیم) از طریق `http://localhost:3001` در دسترس خواهد بود.

برای مشاهده لاگ‌های سرویس‌ها می‌توانید از دستور `docker-compose logs -f` و برای متوقف کردن تمامی سرویس‌ها از `docker-compose down` استفاده کنید.
